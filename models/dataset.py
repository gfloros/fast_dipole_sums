...
class Dataset:
    def __init__(self, conf):
        ...
        self.images = [torch.from_numpy(image.astype(np.float32)).cpu() for image in self.images_np]  # [n_images, H, W, 3]
        self.intrinsics_all = torch.stack(self.intrinsics_all).to(self.device)   # [n_images, 4, 4]
        self.intrinsics_all_inv = torch.inverse(self.intrinsics_all)  # [n_images, 4, 4]
        self.focal = self.intrinsics_all[0][0, 0]
        self.pose_all = torch.stack(self.pose_all).to(self.device)  # [n_images, 4, 4]
        # self.H, self.W = self.images.shape[1], self.images.shape[2]
        self.H = [image.shape[0] for image in self.images]
        self.W = [image.shape[1] for image in self.images]

        print(len(self.intrinsics_all_inv))
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color], dim=-1).cuda()    # batch_size, 7

    def near_far_from_sphere(self, rays_o, rays_d, bounding_sphere=1.0):
        a = torch.sum(rays_d**2, dim=-1, keepdim=True)
        b = 2.0 * torch.sum(rays_o * rays_d, dim=-1, keepdim=True)
        mid = 0.5 * (-b) / a
        near = mid - bounding_sphere
        far = mid + bounding_sphere

        near = torch.clip(near, 0.01, None)
        return near, far
